...
class DimmerLight extends LoxoneDevice {
    ...
    constructor(structureSection: any, platform: LoxonePlatform) {
        super(structureSection, platform, [dimmableLight, bridgedNode, powerSource],
            [
                structureSection.states.position
            ], "dimmable light", `${DimmerLight.name}-${structureSection.uuidAction}`);

        let initialValue = this.latestInitialValueEvent ? Math.round(this.latestInitialValueEvent.value * 2.54) : 0;

        this.Endpoint
            .createDefaultOnOffClusterServer(initialValue !== 0)
            .createDefaultLevelControlClusterServer(initialValue);
