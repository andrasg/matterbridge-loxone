...
class LightMood extends LoxoneDevice {
    ...
    constructor(structureSection: any, platform: LoxonePlatform, moodId: number, moodName: string) {
        super(structureSection, platform, [onOffLight, bridgedNode, powerSource],
            [
                structureSection.states.activeMoods,
                structureSection.states.moodList
            ], "light mood", `${LightMood.name}-${structureSection.uuidAction}-${moodId}`, moodName);
        ...
        this.addLoxoneCommandHandler('on', () => { return `addMood/${this.moodId}`; });
        this.addLoxoneCommandHandler('off', () => { return `removeMood/${this.moodId}`; });
    }
